...
export default function AIAssistant({ 
    ...
    onSuccess: (data) => {
      if (data?.success && data.suggestions) {
        ...
        setChatMessages(prev => [
          ...prev, 
          { sender: 'AI', message: "Here are some suggestions to improve your resume:" },
          ...newMessages
        ]);
        ...
        // Add fallback suggestions to chat
  ...
  const { mutate: generateSummary, isPending: isGeneratingSummary } = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.success && data.suggestions && Array.isArray(data.suggestions)) {
        ...
        setChatMessages(prev => [
          ...prev, 
          { sender: 'AI', message: "Here are some professional summary suggestions:" },
          ...newMessages
        ]);
      } else {
        setChatMessages(prev => [
  ...
  const { mutate: generateBulletPoints, isPending: isGeneratingBullets } = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.success && data.suggestions && Array.isArray(data.suggestions)) {
        ...
        setChatMessages(prev => [
          ...prev, 
          { sender: 'AI', message: "Here are some achievement-focused bullet points:" },
          ...newMessages
        ]);
      } else {
        setChatMessages(prev => [
  ...
  const { mutate: generateSkills, isPending: isGeneratingSkills } = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.success && data.suggestions && Array.isArray(data.suggestions)) {
        ...
        setChatMessages(prev => [
          ...prev, 
          { sender: 'AI', message: "Here are some in-demand skills you might want to add:" },
  ...
  const { mutate: tailorResume, isPending: isTailoring } = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.success && data.tailoredContent) {
        ...
        // Add tailored content to chat
        setChatMessages(prev => [
          ...prev, 
          { sender: 'AI', message: "I've tailored your resume for the job. Here's an improved summary:" },
  ...
  const handleApplyMessage = (message: string, type?: string) => {
    ...
    if (type === 'summary' && onApplySummary) {
      ...
      // For skills that are comma-separated
      if (message.includes(',')) {
